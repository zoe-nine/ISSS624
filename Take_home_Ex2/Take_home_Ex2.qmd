...
The code chunk below shows the spatial distribution of high usage and low usage waterpoint rate (%) at LGA level.
...
It could be seen from the correlation plot that pct\_`mechanised` and pct\_`highusage` are highly positively correlated. Also, pct\_`handpump` and pct\_`lowusage` are highly positively correlated.On the other hand, pct\_`mechanised` and pct\_`lowusage` are highly negatively correlated. `pct_lowusage` and `pct_highusage also have`high negative correlation. Hence we would drop variable `pct_lowusage for this exercise.`
...
As previously mentioned, we have excluded variable pct_highusage and pct_lowusage because they are highly correlated with variable pct_handpump and pct_mechanised.
...
Notice that the row number has been replaced into the township name. Now, we will delete the shapeName field by using the code chunk below.
...
The code chunk below will be used to transform *shan_ict* data frame into a data matrix to make the heatmap.
...
In order to visualise the clusters, the *groups* object need to be appended onto *nga_wp_1* simple feature object.

The code chunk below form the join in three steps:

-   the *groups* list object will be converted into a matrix;

-   *cbind()* is used to append *groups* matrix onto nga_wp_1 to produce an output simple feature object called `nga_wp_1_cluster`; and
...
Next, *st_centroid()* function is used to retrieve the polygon centroid and *sknn2nb()* function will be used to compute the neighbours list using k-nearest neighbors from polygon list.
